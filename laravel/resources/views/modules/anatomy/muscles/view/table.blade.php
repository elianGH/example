    ...
    <table class="mdc-data-table__table" aria-label="Dessert calories">
        ...
        <tbody class="mdc-data-table__content">
            @foreach($table->getBodyData() as $key => $item)
                <tr data-row-id="{{ $key }}" class="mdc-data-table__row mdc-data-table__row--has-actions mdc-data-table__row-col-{{ $table->getColsCount() }}">
                    ...
                    @component('components.blueprints.table.body.cols.body-col--actions', ['area' => $table->area])
                        <a class="mdc-icon-button material-icons" href="{{ route('muscle.edit', $item->id) }}">
                            edit
                        </a>
                    @endcomponent
                </tr>
